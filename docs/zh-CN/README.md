...
[JoJo Codex Pet 正式目录站](https://pixelstand.pet/) 现为可索引的正式站与 canonical 域名。默认英文，可切换简体中文。

站点公开累计宠物详情页浏览次数，并按天汇总成功复制安装命令与点击“在 Codex 中打开”的匿名次数。这些动作只表示安装意图，不代表安装完成，也不是独立用户数。第一方计数器不创建访客标识，也不保留 Cookie、IP、User-Agent、来源页或查询参数。站点不发送安装器完成事件，不接入广告分析，也不渲染广告位；目前只包含站长的 Google AdSense 账号验证元数据。
